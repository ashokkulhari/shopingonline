...
import com.icxl.shopingonline.model.CheckoutModel;
import com.icxl.shopingonline.model.OrderDetail;
import com.icxl.shopingonline.model.OrderItem;
import com.icxl.shopingonline.model.Product;
import com.icxl.shopingonline.model.User;
import com.icxl.shopingonline.model.UserModel;
import com.icxl.shopingonline.service.CartLineService;
...
public class CheckoutHandler {
	...
	public List<Address> getShippingAddresses(CheckoutModel model) {
		...
		List<Address> addresses = userService.listShippingAddresses(model.getUser().getId());
		
		if(addresses.size() == 0) {
			addresses = new ArrayList<>();
		}

		addresses.add(addresses.size(), userService.getBillingAddress(model.getUser().getId()));			
	...
	public String saveAddressSelection(CheckoutModel checkoutModel, int shippingId) {
		...
		Address shipping = userService.getAddressById(shippingId);		
	...
	public String saveAddress(CheckoutModel checkoutModel, Address shipping) {
		...
		userService.saveAddress(shipping);
